...
From the graph, total obesity rate increases persistently from 2011 to 2017. As we can see, the overall male obesity rate is higher than female. 
...
plot6 <- ggplot(data = data_nation_ob_edu, aes(x=YearStart, y=Data_Value,
  ...
  labs(x = "Year",y = "Obesity Rate",
       title = "Obesity Rate from 2011 to 2017 by Educaiton")  + 
  scale_color_brewer(palette="Blues") +
  theme_minimal()
ggplotly(plot6)
```

People with higher degree tend to have lower obesity rate, and the obesity rate of people with colledge degree is the lowest. 
...
plot7 <- ggplot(data = data_nation_ob_age, aes(x=YearStart, y=Data_Value,
  ...
  labs(x = "Year",y = "Obesity Rate",
       title = "Obesity Rate from 2011 to 2017 by age") + 
  scale_color_brewer(palette="Blues")+
  theme_minimal()
ggplotly(plot7)
```

This graph shows that middle-age groups from 35 to 64 tend to have the highest obsesity rate, while the young adults with age from 18 to 24 have the lowest obesity rate. 
...
plot8 <- ggplot(data = data_nation_ob_inc, aes(x=YearStart, y=Data_Value,
  ...
  labs(x = "Year",y = "Obesity Rate",
       title = "Obesity Rate from 2011 to 2017  by income") + 
  scale_color_brewer(palette="Blues")+
  theme_minimal()
ggplotly(plot8)
```
...
plot10 <- ggplot(data_ob,aes(x = reorder(LocationDesc,Data_Value),
  ...
  theme_minimal(8) +
  labs(x = "State", y = "Obesity Rate",title = "States in the Percentage of Obesity in 2017") +
...
data_nation_ac <- data_nation %>%
  filter(QuestionID == "Q043") %>%
    filter(!is.na(Total)|!is.na(Gender))

data_nation_ac$Gender <- ifelse(is.na(data_nation_ac$Gender), data_nation_ac$Total, data_nation_ac$Gender)
...
plot13 <- ggplot(data = data_nation_ac, aes(x=YearStart, y=Data_Value,group=Gender)) +
  geom_line(aes(color=Gender))+labs(x = "Year",
                   y = "Physical Activities Rate",
                   title = "Percent of Adults Who Achieve Physical Activity")+
  scale_color_manual(values=c("#FFDAB9","#1E90FF","black")) +
  theme_minimal()
ggplotly(plot13)
```

The total physical activities rate is falling from 2011 to 2017. Physical activity rate for male is higher than female.

#### State in the Percentage of Physical Activities in 2017
...
plot17 <- ggplot(data_ve,aes(x = reorder(LocationDesc,Data_Value),
                                    y = Data_Value,fill = Data_Value)) +
  geom_bar(stat = 'identity') +
  geom_label(aes(label = Data_Value)) +
  theme_minimal(8) +
  labs(x = "State", y = "Vegetables Consuming Rate",
title = "States in the percentage of obesity adults who report consuming vegetables less than one time daily in 2017") +
  scale_fill_gradient(low = "#FFDAB9",high = "#1E90FF") +
  guides(fill = FALSE) +
  coord_flip()
ggplotly(plot17)
...
Puerto Rico has the highest percentage of obesity adults who report consuming vegetables less than one time daily in 2017, while Maine has the lowest.
